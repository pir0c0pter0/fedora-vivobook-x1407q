...
19. **Câmera IR (HM1092)** — driver próprio `hm1092` + nó no overlay; streaming 560×360 Y10 a ~29,7 fps por `csiphy0 → csid0 → vfe0_rdi0`. AVDD do pm8010 em **2.912 V** (valor do `CAMI_RES_QRD.bin`, o único que cai na grade de 8 mV do RPMh), 1 lane @ 180 MHz, sequência de init de 185 registradores extraída do dump Windows. Iluminador IR no PM8550 (`leds-qcom-flash`, fontes 1+4) limitado aos 700 mA do BSP ASUS; o driver liga somente depois do sensor entrar em stream e desliga antes de pará-lo ou em qualquer erro/power-off
...
| Câmera IR | Hynix HM1092 (Windows Hello), ACPI QCOM0C99 (Spectra 695 ISP Aux Sensor), CCI0/i2c-9 addr 0x24, MCLK0 GPIO 96, reset GPIO 109 |

## TODO

- **Câmera IR (HM1092)** — RESOLVIDO e validado em hardware em 2026-08-31, incluindo o iluminador. A conclusão antiga de que o pm8010 não existia estava errada: o LDO7 falhava porque 2.900.000 µV (do `CAMI_RES_MTP.bin` genérico) não cai na grade de 8 mV do `pmic5_pldo`; o `CAMI_RES_QRD.bin`, que é o que o Windows de fábrica carrega, pede 2.912.000 µV e registra. O flash IR usa o controlador PM8550 a 700 mA nas fontes 1+4 e acompanha automaticamente o stream. Resta integração nativa com o soft-ISP do libcamera; captura funcional em `tools/ir-camera-capture.sh`. Detalhes em `docs/research/CAMERA_STATUS.md`.
