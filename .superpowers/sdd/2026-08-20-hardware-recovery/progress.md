...
Post-reboot checkpoint (boot `4a6f2816-8a54-4822-8ec2-644ebb4c7e7d`): ADSP/CDSP are running, battery/UPower report real values, GPU rendering, Bluetooth, keyboard, touchpad, backlight, hotkeys, audio, cpufreq, charge limit, camera availability, color control, and lid safety pass. Audit false negatives were corrected for remoteproc state output, qcom-battmgr's energy-based ABI, decimal UPower percentages, and unprivileged libinput. Camera/color modules were compiled for and installed into kernel `7.2.0-x1407qa` without loading the camera; color loaded at identity. The GNOME extension is installed with `pending-login` activation. Wi-Fi remains the only hardware failure: PCIe enumerates and both WCN GPIOs are high, but ath11k MHI times out at boot. Because upstream pwrseq explicitly warns against toggling WLAN low after enumeration, the next safe discriminator is a complete power-off/cold boot rather than a live GPIO or PCI reset.
